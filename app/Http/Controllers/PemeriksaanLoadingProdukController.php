...
class PemeriksaanLoadingProdukController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('qc-sistem.pemeriksaan-loading-produk.index', compact('pemeriksaans', 'produkKategoriOptions', 'produkList', 'produkByKategori', 'produkNamaById'));
    }
    ...
    public function exportPDF(Request $request)
    {
        ...
        if ($uuid) {
            $pemeriksaan = PemeriksaanLoadingProduk::where('uuid', $uuid)
                ->with([
                    'user.role',
                    'user.plant',
                    'shift',
            ...
            // Build produkMap and tujuanMap for PDF view
            $produkIds = collect($pemeriksaan->produk_data ?? [])
                ->pluck('id_produk')
                ->filter()
                ->unique()
                ->all();
            ...
            $tujuanIds = collect($pemeriksaan->produk_data ?? [])
                ->pluck('id_tujuan_pengiriman')
                ->filter()
                ->unique()
                ->all();
            ...
            return $pdf->download('loading-produk-' . $pemeriksaan->uuid . '.pdf');
        }
    ...
    public function batchVerify(Request $request)
    {
        ...
        $query = PemeriksaanLoadingProduk::query();
