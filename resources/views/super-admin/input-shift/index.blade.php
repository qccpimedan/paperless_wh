        ...
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    <h3>Data Shift</h3>
                    <p class="text-subtitle text-muted">Kelola data shift kerja</p>
                </div>
                            ...
                            <tbody>
                                @forelse($shifts as $index => $shift)
                                    <tr>
                                        ...
                                        <td>
                                            @if($shift->user->plant)
                                                <span class="badge bg-info">{{ $shift->user->plant->plant }}</span>
                                            @else
                                        ...
                                        <td>
                                            <div class="btn-vertical" role="group">
                                                <a href="{{ route('shifts.edit', $shift->uuid) }}" class="btn btn-sm btn-warning">
                                                    <i class="bi bi-pencil"></i>
                                                </a>
                                                <form action="{{ route('shifts.destroy', $shift->uuid) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
