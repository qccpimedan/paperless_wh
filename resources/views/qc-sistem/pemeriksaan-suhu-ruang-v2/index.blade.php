...
<div id="main">
        ...
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    <h3>Pemeriksaan Suhu Ruang (CS Meat)</h3>
                    <p class="text-subtitle text-muted">Kelola data pemeriksaan suhu ruang cold storage meat</p>
                </div>
                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Suhu Ruang v2</li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('pemeriksaan-suhu-ruang-v2.index') }}" method="GET" class="row g-3 mb-3">
                        <div class="col-md-6"><input type="text" name="search" class="form-control" placeholder="Cari Produk..." value="{{ request('search') }}"></div>
                        ...
                        <div class="col-md-3 d-flex align-items-end gap-2">
                            ...
                            <a href="{{ route('pemeriksaan-suhu-ruang-v2.index') }}" class="btn btn-secondary btn-sm">Reset</a>
                        </div>
                    </form>

                    <form id="batchActionForm" action="{{ route('pemeriksaan-suhu-ruang-v2.batch-verify') }}" method="POST">
                        @csrf
                                ...
                                <tbody>
                                    @forelse($pemeriksaans as $index => $item)
                                        ...
                                        <tr>
                                            ...
                                            @endif
                                            <td><span class="badge bg-info">{{ $item->produk->nama_produk ?? '-' }}</span></td>
                                            <td>
                                                @if($st === 'pending' || $st === null)
                                                    @if($userRole === 'qc inspector')
                                                        <button type="submit" class="btn btn-sm btn-primary" formaction="{{ route('pemeriksaan-suhu-ruang-v2.send-to-produksi', $item->uuid) }}"><i class="bi bi-send"></i> Kirim</button>
                                            ...
                                            <td>
                                                <div class="btn-vertical">
                                                    @can('view_pemeriksaan_suhu_ruang_v2') <a href="{{ route('pemeriksaan-suhu-ruang-v2.show', $item->uuid) }}" class="btn btn-sm btn-info text-white"><i class="bi bi-eye"></i></a> @endcan
                                                    @can('edit_pemeriksaan_suhu_ruang_v2') <a href="{{ route('pemeriksaan-suhu-ruang-v2.edit', $item->uuid) }}" class="btn btn-sm btn-warning text-white"><i class="bi bi-pencil"></i></a> @endcan
                                                    ...
                                                    @endcan
                                                    <a href="{{ route('pemeriksaan-suhu-ruang-v2.edit', $item->uuid) }}?edit_per_2jam=1" 
                                                        class="btn btn-sm btn-success" title="Edit Per Jam">
                                                        <i class="bi bi-hourglass-bottom"></i>
                                                    </a>
                                                    @can('view_pemeriksaan_suhu_ruang_v2')
                                                    <a href="{{ route('pemeriksaan-suhu-ruang-v2.history', $item->uuid) }}" 
                                                        class="btn btn-sm btn-secondary" title="Lihat History">
                                                        <i class="bi bi-clock-history"></i>
                                                    </a>
                                                    @endcan
                                                    <!-- <a href="{{ route('pemeriksaan-suhu-ruang-v2.print-pdf', $item->uuid) }}" 
                                    ...
                                    @empty
                                        <tr><td colspan="{{ ($isPlantMedanHeader || $isSuperadminHeader) ? 9 : 8 }}" class="text-center py-4">Belum ada data</td></tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
            ...
            <div class="modal fade" id="multiApprovalModal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form action="{{ route('pemeriksaan-suhu-ruang-v2.batch-verify') }}" method="POST">
                            @csrf
    ...
    @foreach($pemeriksaans as $item)
        @can('delete_pemeriksaan_suhu_ruang_v2')
            <form id="del-{{$item->uuid}}" action="{{ route('pemeriksaan-suhu-ruang-v2.destroy', $item->uuid) }}" method="POST" style="display:none;">
                @csrf
                @method('DELETE')
            </form>
        @endcan
