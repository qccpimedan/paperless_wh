...
<div id="main">
        ...
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    ...
                    <p class="text-subtitle text-muted">Kelola data pemeriksaan kedatangan bahan baku dan penunjang</p>
                </div>
                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Bahan Baku & Penunjang</li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row mb-4 p-3 bg-light rounded">
                        ...
                        <form action="{{ route('pemeriksaan-bahan-baku.export-pdf') }}" method="GET" class="row g-3" id="filterForm">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3 d-flex align-items-end gap-2">
                                ...
                                @if($canVerify)
                                    <button type="submit" class="btn btn-primary flex-grow-1" formaction="{{ route('pemeriksaan-bahan-baku.batch-verify') }}" formmethod="POST" onclick="return confirm('Verifikasi semua data pada filter ini?')">
                    ...
                    <form id="batchActionForm" action="{{ route('pemeriksaan-bahan-baku.batch-verify') }}" method="POST">
                        @csrf
                                ...
                                <tbody>
                                    @forelse($pemeriksaans as $index => $item)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @php
                                                    $kodeArray = json_decode($item->kode_produksi_array ?? '[]', true);
                                                    $kodeProduksi = is_array($kodeArray) && count($kodeArray) > 0 ? $kodeArray[0] : ($item->kode_produksi ?? '-');
                                                @endphp
                                                {{ $kodeProduksi }}
                                            ...
                                            <td>
                                                @if($st === 'pending' || $st === null)
                                                    @if($userRole === 'qc inspector')
                                                        <button type="submit" class="btn btn-sm btn-primary" formaction="{{ route('pemeriksaan-bahan-baku.send-to-produksi', $item->uuid) }}"><i class="bi bi-send"></i> Kirim</button>
                                            ...
                                            <td><small>{{ Str::limit($item->verification_notes ?? '-', 15) }}</small></td>
                                            <td>
                                                <div class="btn-vertical">
                                                    @can('view_pemeriksaan_kedatangan_bahan_baku_penunjang') <a href="{{ route('pemeriksaan-bahan-baku.show', $item->uuid) }}" class="btn btn-sm btn-info text-white"><i class="bi bi-eye"></i></a> @endcan
                                                    @can('edit_pemeriksaan_kedatangan_bahan_baku_penunjang') <a href="{{ route('pemeriksaan-bahan-baku.edit', $item->uuid) }}" class="btn btn-sm btn-warning text-white"><i class="bi bi-pencil"></i></a> @endcan
    ...
    @foreach($pemeriksaans as $item)
        @can('delete_pemeriksaan_kedatangan_bahan_baku_penunjang')
            <form id="del-{{$item->uuid}}" action="{{ route('pemeriksaan-bahan-baku.destroy', $item->uuid) }}" method="POST" style="display:none;">
                @csrf
                @method('DELETE')
            </form>
        @endcan
