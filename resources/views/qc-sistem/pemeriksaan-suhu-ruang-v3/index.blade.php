...
<div id="main">
        ...
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    <h3>Pemeriksaan Suhu Ruang (Gudang Dry)</h3>
                    <p class="text-subtitle text-muted">Kelola data pemeriksaan suhu ruang gudang dry</p>
                </div>
                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Suhu Ruang v3</li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        @endif
                        @can('create_pemeriksaan_suhu_ruang_v3')
                ...
                <div class="card-body">
                    <div class="row mb-4 p-3 bg-light rounded">
                        ...
                        <form action="{{ route('pemeriksaan-suhu-ruang-v3.export-pdf') }}" method="GET" class="row g-3" id="filterForm">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3 d-flex align-items-end gap-2">
                                <button type="submit" formaction="{{ route('pemeriksaan-suhu-ruang-v3.export-pdf') }}" class="btn btn-danger w-100"><i class="bi bi-file-pdf"></i> PDF</button>
                                <button type="submit" formaction="{{ route('pemeriksaan-suhu-ruang-v3.export-excel') }}" class="btn btn-success w-100"><i class="bi bi-file-earmark-spreadsheet"></i> Excel</button>
                            </div>
                        </form>
                    </div>
                    ...
                    <form id="batchActionForm" action="{{ route('pemeriksaan-suhu-ruang-v3.batch-verify') }}" method="POST">
                        @csrf
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($st === 'pending' || $st === null)
                                                    @if($userRole === 'qc inspector')
                                                        <button type="submit" class="btn btn-sm btn-primary" formaction="{{ route('pemeriksaan-suhu-ruang-v3.send-to-produksi', $item->uuid) }}"><i class="bi bi-send"></i> Kirim</button>
                                            ...
                                            <td>
                                                <div class="btn-vertical">
                                                    @can('view_pemeriksaan_suhu_ruang_v3') <a href="{{ route('pemeriksaan-suhu-ruang-v3.show', $item->uuid) }}" class="btn btn-sm btn-info text-white"><i class="bi bi-eye"></i></a> @endcan
                                                    @can('edit_pemeriksaan_suhu_ruang_v3') <a href="{{ route('pemeriksaan-suhu-ruang-v3.edit', $item->uuid) }}" class="btn btn-sm btn-warning text-white"><i class="bi bi-pencil"></i></a> @endcan
                                                    ...
                                                    @endcan
                                                    <a href="{{ route('pemeriksaan-suhu-ruang-v3.edit', $item->uuid) }}?edit_per_2jam=1" 
                                                        class="btn btn-sm btn-success" title="Edit Per Jam">
                                                        <i class="bi bi-hourglass-bottom"></i>
                                                    </a>
                                                    @can('view_pemeriksaan_suhu_ruang_v3')
                                                    <a href="{{ route('pemeriksaan-suhu-ruang-v3.history', $item->uuid) }}" 
                                                        class="btn btn-sm btn-secondary" title="Lihat History">
                                                        <i class="bi bi-clock-history"></i>
                                                    </a>
                                                    @endcan
                                                    <!-- <a href="{{ route('pemeriksaan-suhu-ruang-v3.print-pdf', $item->uuid) }}" 
            ...
            <div class="modal fade" id="multiApprovalModal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form action="{{ route('pemeriksaan-suhu-ruang-v3.batch-verify') }}" method="POST">
                            @csrf
    ...
    @foreach($pemeriksaans as $item)
        @can('delete_pemeriksaan_suhu_ruang_v3')
            <form id="del-{{$item->uuid}}" action="{{ route('pemeriksaan-suhu-ruang-v3.destroy', $item->uuid) }}" method="POST" style="display:none;">
                @csrf
                @method('DELETE')
            </form>
        @endcan
