...
<div id="main">
        ...
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    <h3>Pemeriksaan Suhu Ruang (Gudang CS)</h3>
                    <p class="text-subtitle text-muted">Kelola data pemeriksaan suhu ruang gudang cold storage</p>
                </div>
                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Suhu Ruang</li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row mb-4 p-3 bg-light rounded">
                        ...
                        <form action="{{ route('pemeriksaan-suhu-ruang.export-pdf') }}" method="GET" class="row g-3" id="filterForm">
                            <div class="col-md-3">
                    ...
                    <form action="{{ route('pemeriksaan-suhu-ruang.index') }}" method="GET" class="row g-3 mb-3">
                        ...
                        <div class="col-md-3 d-flex align-items-end gap-2">
                            ...
                            <a href="{{ route('pemeriksaan-suhu-ruang.index') }}" class="btn btn-secondary btn-sm">Reset</a>
                        </div>
                    </form>

                    <form id="batchActionForm" action="{{ route('pemeriksaan-suhu-ruang.batch-verify') }}" method="POST">
                        @csrf
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($st === 'pending' || $st === null)
                                                    @if($userRole === 'qc inspector')
                                                        <button type="submit" class="btn btn-sm btn-primary" formaction="{{ route('pemeriksaan-suhu-ruang.send-to-produksi', $item->uuid) }}"><i class="bi bi-send"></i> Kirim</button>
                                            ...
                                            <td>
                                                <div class="btn-vertical">
                                                    @can('view_pemeriksaan_suhu_ruang') <a href="{{ route('pemeriksaan-suhu-ruang.show', $item->uuid) }}" class="btn btn-sm btn-info text-white"><i class="bi bi-eye"></i></a> @endcan
                                                    @can('edit_pemeriksaan_suhu_ruang') <a href="{{ route('pemeriksaan-suhu-ruang.edit', $item->uuid) }}" class="btn btn-sm btn-warning text-white"><i class="bi bi-pencil"></i></a> @endcan
            ...
            <div class="modal fade" id="multiApprovalModal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form action="{{ route('pemeriksaan-suhu-ruang.batch-verify') }}" method="POST">
                            @csrf
    ...
    @foreach($pemeriksaans as $item)
        @can('delete_pemeriksaan_suhu_ruang')
            <form id="del-{{$item->uuid}}" action="{{ route('pemeriksaan-suhu-ruang.destroy', $item->uuid) }}" method="POST" style="display:none;">
                @csrf
                @method('DELETE')
            </form>
        @endcan
