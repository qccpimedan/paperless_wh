...
<div id="main">
        ...
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    <h3>Pemeriksaan Loading Produk</h3>
                ...
                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Loading Produk</li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row mb-4 p-3 bg-light rounded">
                        ...
                        <form action="{{ route('pemeriksaan-loading-produk.export-pdf') }}" method="GET" class="row g-3" id="filterForm">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3 d-flex align-items-end gap-2">
                                ...
                                @if($canVerify)
                                    <button type="submit" class="btn btn-primary flex-grow-1" formaction="{{ route('pemeriksaan-loading-produk.batch-verify') }}" formmethod="POST" onclick="return confirm('Verifikasi semua data pada filter ini?')">
                    ...
                    <form id="batchActionForm" action="{{ route('pemeriksaan-loading-produk.batch-verify') }}" method="POST">
                        @csrf
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($st === 'pending' || $st === null)
                                                    @if($userRole === 'qc inspector')
                                                        <button type="submit" class="btn btn-sm btn-primary" formaction="{{ route('pemeriksaan-loading-produk.send-to-produksi', $item->uuid) }}"><i class="bi bi-send"></i> Kirim</button>
                                            ...
                                            <td>
                                                <div class="btn-vertical">
                                                    @can('view_pemeriksaan_loading_produk') <a href="{{ route('pemeriksaan-loading-produk.show', $item->uuid) }}" class="btn btn-sm btn-info text-white"><i class="bi bi-eye"></i></a> @endcan
                                                    @can('edit_pemeriksaan_loading_produk') <a href="{{ route('pemeriksaan-loading-produk.edit', $item->uuid) }}" class="btn btn-sm btn-warning text-white"><i class="bi bi-pencil"></i></a> @endcan
    ...
    @foreach($pemeriksaans as $item)
        @can('delete_pemeriksaan_loading_produk')
            <form id="del-{{$item->uuid}}" action="{{ route('pemeriksaan-loading-produk.destroy', $item->uuid) }}" method="POST" style="display:none;">
                @csrf
                @method('DELETE')
            </form>
        @endcan
