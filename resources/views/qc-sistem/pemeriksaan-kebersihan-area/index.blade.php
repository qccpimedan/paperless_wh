...
<div id="main">
        ...
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    ...
                    <p class="text-subtitle text-muted">Kelola data pemeriksaan kebersihan area</p>
                </div>
                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Kebersihan Area</li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row mb-4 p-3 bg-light rounded">
                        ...
                        <form action="{{ route('pemeriksaan-kebersihan-area.export-pdf') }}" method="GET" class="row g-3" id="filterForm">
                            <div class="col-md-3">
                    ...
                    <form action="{{ route('pemeriksaan-kebersihan-area.index') }}" method="GET" class="row g-3 mb-3">
                        <div class="col-md-9"><input type="text" name="search" class="form-control" placeholder="Cari Area..." value="{{ request('search') }}"></div>
                        ...
                        <div class="col-md-3 d-flex align-items-end gap-2">
                            ...
                            <a href="{{ route('pemeriksaan-kebersihan-area.index') }}" class="btn btn-secondary btn-sm">Reset</a>
                        </div>
                    </form>

                    <form id="batchActionForm" action="{{ route('kebersihan-area.batch-verify') }}" method="POST">
                        @csrf
                                ...
                                <tbody>
                                    @forelse($pemeriksaans as $index => $item)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @php
                                                    $areas = collect($item->area_data)->pluck('id_area')->filter()->unique()->values()->all();
                                                    $countArea = count($areas);
                                                    $firstAreaId = $areas[0] ?? null;
                                                    $firstArea = $firstAreaId ? ($areaNamaById[$firstAreaId] ?? '-') : '-';
                                                @endphp
                                                <span class="badge bg-info">{{ $firstArea }}</span>
                                                @if($countArea > 1) <br><small class="text-primary">+{{ $countArea-1 }} lainnya</small> @endif
                                            </td>
                                            <td>
                                                @php
                                                    $forms = collect($item->area_data)->pluck('id_master_form')->filter()->unique()->values()->all();
                                                    $countForm = count($forms);
                                                    $firstFormId = $forms[0] ?? null;
                                                    $firstForm = $firstFormId ? ($masterFormNamaById[$firstFormId] ?? '-') : '-';
                                                @endphp
                                                <span class="badge bg-secondary">{{ $firstForm }}</span>
                                                @if($countForm > 1) <br><small class="text-primary">+{{ $countForm-1 }} lainnya</small> @endif
                                            </td>
                                            <td>
                                                @if($st === 'pending' || $st === null)
                                                    @if($userRole === 'qc inspector')
                                                        <button type="submit" class="btn btn-sm btn-primary" formaction="{{ route('kebersihan-area.send-to-produksi', $item->uuid) }}"><i class="bi bi-send"></i> Kirim</button>
                                            ...
                                            <td>
                                                <div class="btn-vertical">
                                                    @can('view_pemeriksaan_kebersihan_area') <a href="{{ route('pemeriksaan-kebersihan-area.show', $item->uuid) }}" class="btn btn-sm btn-info text-white"><i class="bi bi-eye"></i></a> @endcan
                                                    @can('edit_pemeriksaan_kebersihan_area') <a href="{{ route('pemeriksaan-kebersihan-area.edit', $item->uuid) }}" class="btn btn-sm btn-warning text-white"><i class="bi bi-pencil"></i></a> @endcan
                                    ...
                                    @empty
                                        <tr><td colspan="{{ ($isPlantMedanHeader || $isSuperadminHeader) ? 10 : 9 }}" class="text-center py-4">Belum ada data</td></tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
            ...
            <div class="modal fade" id="multiApprovalModal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form action="{{ route('kebersihan-area.batch-verify') }}" method="POST">
                            @csrf
    ...
    @foreach($pemeriksaans as $item)
        @can('delete_pemeriksaan_kebersihan_area')
            <form id="del-{{$item->uuid}}" action="{{ route('pemeriksaan-kebersihan-area.destroy', $item->uuid) }}" method="POST" style="display:none;">
                @csrf
                @method('DELETE')
            </form>
        @endcan
